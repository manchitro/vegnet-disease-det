...
class MobileNetV2_dynamicFPN(nn.Module):
    def __init__(self, width_mult=1., num_classes=3, num_anchors=9):
        ...
        # Additional layers for bounding box and class predictions
        self.bbox_layer = nn.ModuleList([nn.Conv2d(256, 4, kernel_size=3, stride=1, padding=1) for _ in range(len(self.smooth_layers))])
        self.class_layer = nn.ModuleList([nn.Conv2d(256, self.num_classes, kernel_size=3, stride=1, padding=1) for _ in range(len(self.smooth_layers))])
    ...
    def forward(self, x):
        ...
        bbox_preds = [bbox_layer(p_layer) for bbox_layer, p_layer in zip(self.bbox_layer, p_layers)]
        class_preds = [class_layer(p_layer) for class_layer, p_layer in zip(self.class_layer, p_layers)]

        return p_layers, bbox_preds, class_preds
